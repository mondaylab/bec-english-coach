...
12. 新建文档后放入学习者指定的飞书知识库栏目；若已有固定栏目链接，沿用该栏目，不要每天重新询问。当前固定栏目链接：`https://jwf86xh1hew.feishu.cn/wiki/W2NYwEFZxi9JADky5NdcHqWpnpH`。
13. 飞书文档二级标题统一使用 `一、`、`二、`、`三、` 作为前置序号，例如 `## 一、正文草稿`。
14. 飞书文档默认包含：正文草稿、图文文字版、版式与图片状态。图片生成后按 card-01 到 card-08 顺序插入最后一部分。
15. 仓库要及时更新：练习答案确认后写入 `practice/`；公众号标题、正文草稿或卡片文字确认后写入 `publications/wechat/YYYY/MM/{day-slug}/`；飞书文档创建或图片生成后，把链接、路径和状态同步回对应记录。
