...
7. 写观点时避免大量使用“不是……而是……”或“不在于……而在于……”。优先直接写后半句的正向观点；如果前半句的信息也有价值，把它放在观点后面作为补充条件、反例或边界，不要让读者先做一次否定判断。
8. 等学习者确认或修改正文草稿、卡片文字、版式参考后，再生成图片。
9. 如果学习者提供版式参考，优先沿用参考图的视觉语言；封面要有一个足够大的中文视觉锤，英文只作为辅助，不要突然变成英语学习卡。
10. 图文卡片默认 8 张：封面大字报、核心判断、用户痛点、已有替代方案、生态位/工作流、付费意愿、策略表达、钩子问题。
11. 最后一张优先做成钩子图，抛出 3-4 个问题让读者继续思考，不只做总结。
12. 若需要写入飞书专栏，每天默认新建一篇日期文档，标题格式使用 `YYYY-MM-DD｜{公众号标题}`；只有学习者明确要求继续修改当天已有文档时，才更新已有文档。
13. 新建文档后放入学习者指定的飞书知识库栏目；若已有固定栏目链接，沿用该栏目，不要每天重新询问。当前固定栏目链接：`https://jwf86xh1hew.feishu.cn/wiki/W2NYwEFZxi9JADky5NdcHqWpnpH`。
14. 飞书文档二级标题统一使用 `一、`、`二、`、`三、` 作为前置序号，例如 `## 一、正文草稿`。
15. 飞书文档默认包含：正文草稿、图文文字版、版式与图片状态。图片生成后按 card-01 到 card-08 顺序插入最后一部分。
16. 仓库要及时更新：练习答案确认后写入 `practice/`；公众号标题、正文草稿或卡片文字确认后写入 `publications/wechat/YYYY/MM/{day-slug}/`；飞书文档创建或图片生成后，把链接、路径和状态同步回对应记录。
